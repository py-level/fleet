...
def test_complex_for_rendering():
    template = '''
    <div class="user-list">
    ...
    assert '<div class="[\'user-card\']">' in result
    assert '<h3>John Doe</h3>' in result
    assert '<h3>Jane Smith</h3>' in result
    
    # Check if roles are rendered
    assert '<p>Developer</p>' in result
    assert '<p>Designer</p>' in result
    
    # Check if skills are rendered
    assert '<li>Python</li>' in result
    assert '<li>JavaScript</li>' in result
    assert '<li>Vue</li>' in result
    assert '<li>UI/UX</li>' in result
    assert '<li>Figma</li>' in result
    assert '<li>CSS</li>' in result 